            ...
            if (isResetMode) {
                ...
                if (result.success) {
                    if (result.recovery_code) {
                        ...
                        // User remains on login page but with modal visible
                        setMessage({ text: 'Registration successful!', type: 'success' });
                    } else {
                        setMessage({
                            text: 'Registration successful! Please log in to secure your account.',
                            type: 'success'
                        });
                        setTimeout(() => {
                            setIsLogin(true);
                ...
                <div className="w-full max-w-md space-y-8 relative z-10">
                    ...
                    {!isResetMode && (
                        <div className={cn("grid grid-cols-1 gap-4", showRecoveryModal && "opacity-20 pointer-events-none")}>
                            {/* Premium Google Button Overlay Container */}
                            <div className="relative group rounded-full overflow-hidden h-12 flex items-center justify-center transition-all duration-300">
                                {/* Our Custom Styled Button (The visible part) */}
                                <div className="absolute inset-0 flex items-center justify-center gap-3 px-6 rounded-full border border-slate-700 bg-slate-950 group-hover:bg-slate-900 transition-all duration-300 pointer-events-none">
                                ...
                                {/* The Real Google Button (Hidden but clickable) */}
                                <div
                                    ref={googleButtonRef}
                                    className="absolute inset-0 z-10 cursor-pointer opacity-[0.01] [&_iframe]:!w-full [&_iframe]:!h-full"
                                ></div>
                            </div>
